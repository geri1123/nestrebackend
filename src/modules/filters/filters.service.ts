...
export class FiltersService implements OnModuleInit {
  ...
  async getCategories(
    ...
    const [structure, { subcategoryCountMap, categoryCountMap }] = await Promise.all([
      this.getCategoryStructure(lang),
      this.categoryRepo.getCategoryCounts(status),
    ]);

    return this.mapCategoriesWithCounts(structure, subcategoryCountMap, categoryCountMap);
  }
  ...
  async getCities(countryCode: string): Promise<CityDto[]> {
    // Cities are dynamic (per countryCode) so we register refresher on first use
