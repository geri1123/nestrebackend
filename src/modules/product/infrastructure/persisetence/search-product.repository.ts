...
export class SearchProductRepository implements ISearchProductRepository {
  ...
  /**
   * FIX: activeAd si getter — krijon new Date() çdo herë që thirret,
   * jo vetëm kur starton serveri (bug me `private readonly activeAd = {...}`).
   */
  private get activeAd() {
    const now = new Date();
    return {
      status: 'active' as const,
      startDate: { lte: now },
  ...
  private getSelect(language: SupportedLang): Prisma.ProductSelect {
    // FIX: advertisements filter gjithashtu përdor now() dinamike
    const now = new Date();
    return {
      id: true,
      clickCount: true,
  ...
  async searchProducts(
    ...
    const limit = filters.limit!;
    const offset = filters.offset!;
    const regularOrderBy = this.getOrderBy(filters.sortBy);

    // Count total ads matching filters
    const totalAds = await this.prisma.product.count({ where: adWhere });

    // How many ads to skip/show on this page
    const adsSkipped    = Math.min(offset, totalAds);
    const adsToShow     = Math.min(Math.max(totalAds - adsSkipped, 0), limit);
    ...
    // Sort ads: premium > normal > cheap
    const adPriority: Record<string, number> = { premium: 0, normal: 1, cheap: 2 };
    const sortedAds = [...adProducts].sort((a, b) => {
      const aType = a.advertisements?.[0]?.adType ?? 'cheap';
      const bType = b.advertisements?.[0]?.adType ?? 'cheap';
      return (adPriority[aType] ?? 2) - (adPriority[bType] ?? 2);
    });

    return [...sortedAds, ...regularProducts];
  }
