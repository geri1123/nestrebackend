...
export class DeleteProductUseCase {
  ...
  async execute(productId: number, lang: SupportedLang): Promise<void> {
    // findById returns the full entity (incl. status, subcategoryId,
    // listingTypeId) which we need both for permission and for the
    // count-decrement event.
    ...
    if (images.length > 0) {
      await Promise.all(
        images
          ...
          .map((img) =>
            this.deleteImages
              .executeByUrls([], [img.publicId!])
              .catch(console.error),
          ),
      );
    }

    // Decrement counts in the background — never blocks the response.
