...
export class AgencyContextGuard implements CanActivate {
  constructor(
    private readonly reflector: Reflector,
    private readonly agencyContextOrchestrator: AgencyContextOrchestrator,
  ) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (this.isContextLoaded(req)) {
      this.agencyContextOrchestrator.validateStatus(req, lang);
      return true;
    }
    ...
    if (!this.shouldLoadContext(req, requireAgencyContext)) {
      return true;
    }

    await this.agencyContextOrchestrator.loadContext(req, lang);
    this.agencyContextOrchestrator.validateStatus(req, lang);
  ...
  private isContextLoaded(req: RequestWithUser): boolean {
    return req.agencyId !== undefined;
  }

  private shouldLoadContext(req: RequestWithUser, requireAgencyContext: boolean): boolean {
    const { user } = req;
